...
#define RTX_SOFTWARE_VERSION {1, 5, 0}
#define RTX_DEVICE_IP 1

#define RTX_EEPROM_SECTORS 6
#define RTX_DEBUG_SECTORS 2

#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\RTX Systems\RTX V2\CORE_Code\AtemControl\AtemControl_1.0.0.h"
  ...
  if (!RTX_PROTOCAL.RTX_EEPROM_ERROR) {
    ...
    //Check if Tally inputs have changed:
    {
      ...
      for (uint8_t X = 0; X < 8 && !TallyChange; X++) {
        if (ATEM_PrevTally[X] != ATEM.tally(X)) {
          ATEM_PrevTally[X] = ATEM.tally(X);
          TallyChange = true;
        }
      }
      if (TallyChange) {
    ...
    //Update Tally Debug Registers:
    {
      ...
      for (uint8_t X = 0; X < 8; X++) {
        DEBUG_BUFF(3) |= (ATEM.tally(X) & B00000001) << X;
        DEBUG_BUFF(4) |= ((ATEM.tally(X) & B00000010) >> 1) << X;
      }
    }
...
void RTX_Send() {
  ...
  for (uint8_t X = 0; X < 8; X++) {
    TX_MSG[1] |= (ATEM.tally(X) & B00000001) << X;
    TX_MSG[2] |= ((ATEM.tally(X) & B00000010) >> 1) << X;
  }
