...
{
public:
  void setup(IPAddress, uint16_t); // setup here. automatically start connection as well
  bool run();                      // run atem connection. run often enough. returns true if connected, false otherwise
  void connect();                  // use to start or reset connection
  void disconnect();               // use to end connection. connection will stay disconnected until "start" or "connect" is called
