...
class COMP_UIP {
  public:
    ...
    void ConnectionSleep(uint16_t Seconds) {
      _ConnectionSleepDelay = Seconds * 1000;
    }
    uint16_t ConnectionSleep() {
      return ((millis() - _ConnectionSleepTimer) / 1000);
    }
    void ResetSleep() {
  ...
  private:
    ...
    uint16_t _COMP_Port = 51235;//Default Companion UDP Port
    uint16_t _RX_PORT = 8000;
    uint16_t _TX_PORT = 8888;
    uint8_t _ConnectPage = 5;
...
void COMP_UIP::PressButton(uint8_t Page, uint8_t Bank, bool Dir) {
  if (_Init) {
    if (Page > 0 && Page < 100 && Bank > 0 && Bank < 33) {
      ...
      COMP_TX.beginPacket(_COMP_IP, _COMP_Port);
