...
class ONEWIRE_Master {
  ...
  private:
    ...
    uint8_t _SlaveIP[8];
    uint8_t _SlaveCount;
    bool _ConnectedDevice[8] = {false, false, false, false, false, false, false, false};
    uint32_t _ConnectedTimer[8];
};
                ...
                if (Command == 2) {
                  ...
                  if (_CheckIP(RecieverSlaveIP, RecieverIP_TAG)) {
                    ...
                    if (_CheckConnected(RecieverIP_TAG, 2)) {
                      ...
                      BitMarker = 0;
                      BoolConverter.CompileVal(BitArray, BitMarker, _SlaveIP[X], 3);
                      BoolConverter.CompileVal(BitArray, BitMarker, _SlaveBuffer.Write(RecieverIP_TAG, X, ((DeCompileMarker - 8) / 8), DataArray), 1);
