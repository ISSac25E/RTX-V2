...
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\RTX Systems\RTX V2\RTX_CENTER_SWITCHER\RTX_CENTER_SWITCHER_1.3.0\include.h"
...
/*
  all objects for sequence builders and macros:
*/
SequenceBuild StatusLedBuild;
SequenceBuild TallyLedBuild;
SequenceBuild ErrorLedBuild;
...
/*
  values for neo-pixels for macro driver:
*/
uint8_t StatusGreen = 0;
uint8_t StatusRed = 0;
uint8_t StatusBlue = 0;

uint8_t TallyGreen = 0;
uint8_t TallyRed = 0;
uint8_t TallyBlue = 0;

/*
  NeoPixel setup:
    all NeoPixel values will be set using the byte values above. This is needed for the macro driver
*/
NEO_PIXEL_DRIVER PixelDriver(12);

NEO_PIXEL NeoPixel_StatusGreen(&PixelDriver);
NEO_PIXEL NeoPixel_StatusRed(&PixelDriver);
NEO_PIXEL NeoPixel_StatusBlue(&PixelDriver);

NEO_PIXEL NeoPixel_TallyGreen(&PixelDriver);
NEO_PIXEL NeoPixel_TallyRed(&PixelDriver);
NEO_PIXEL NeoPixel_TallyBlue(&PixelDriver);
    ...
    case 0:
      ...
      if (ATEM_CONNECTION)
      {
        if ((ATEM_TALLY_PRG & B00110000) && (ATEM_TALLY_PRG & B00110000) != B00110000)
        {
          ...
          {
            uint8_t TX_MSG[3] = {B00100011, (CONFIG_BUFF(192) & B01111111), 28};
            if (!RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG))
              DEBUG_BUFF(15)
            ++;
            else DEBUG_BUFF(14)++;
          ...
          {
            uint8_t TX_MSG[3] = {B00100011, (CONFIG_BUFF(192) & B01111111), 29};
            if (!RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG))
              DEBUG_BUFF(15)
            ++;
            else DEBUG_BUFF(14)++;
    ...
    case 1:
      ...
      if (millis() - ButtonTimeOut >= TimeOut)
      {
        if (ATEM_CONNECTION)
        {
          ...
          {
            uint8_t TX_MSG[3] = {B00100011, (CONFIG_BUFF(192) & B01111111), 26};
            if (!RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG))
              DEBUG_BUFF(15)
            ++;
            else DEBUG_BUFF(14)++;
          ...
          {
            uint8_t TX_MSG[3] = {B00100011, (CONFIG_BUFF(192) & B01111111), 25};
            if (!RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG))
              DEBUG_BUFF(15)
            ++;
            else DEBUG_BUFF(14)++;
    ...
    case 2:
      ...
      if (millis() - ButtonTimeOut >= TimeOut)
      {
        if (ATEM_CONNECTION)
        {
          if ((ATEM_TALLY_PRG & B00110000))
          {
            uint8_t TX_MSG[3] = {B00100011, (CONFIG_BUFF(192) & B01111111), 27};
            if (!RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG))
              DEBUG_BUFF(15)
            ++;
            else DEBUG_BUFF(14)++;
          ...
          {
            uint8_t TX_MSG[3] = {B00100011, (CONFIG_BUFF(192) & B01111111), 24};
            if (!RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG))
              DEBUG_BUFF(15)
            ++;
            else DEBUG_BUFF(14)++;
...
{
  /*
    run macros and set NeoPixel LED values:
  */
  {
    // run handlers:
    Macro.run();
    ...
    // set neo-pixel led's
    NeoPixel_StatusRed.Set(StatusRed);
    NeoPixel_StatusGreen.Set(StatusGreen);
    NeoPixel_StatusBlue.Set(StatusBlue);

    NeoPixel_TallyRed.Set(TallyRed);
    NeoPixel_TallyGreen.Set(TallyGreen);
    NeoPixel_TallyBlue.Set(TallyBlue);
