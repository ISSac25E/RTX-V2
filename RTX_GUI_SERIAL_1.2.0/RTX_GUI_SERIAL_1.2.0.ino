      ...
      if (ParseNext() == 0) {
        if (CharAvail == 3 && CharComp("PNG", C, CharAvail)) {
          ...
          if (RTX_PROTOCAL.Connected()) {
            ...
            //            noInterrupts();
            Serial.flush();
            bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_PING_LEN, RTX_TX);
            //            interrupts();
            if (WriteStat) {
          ...
          if (RTX_PROTOCAL.Connected()) {
            ...
            //            noInterrupts();
            Serial.flush();
            bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_DEV_NAME_LEN, RTX_TX);
            //            interrupts();
            if (WriteStat) {
              uint32_t RTX_ReadTimeOut = micros();
              char DeviceName[5];
          ...
          if (RTX_PROTOCAL.Connected()) {
            ...
            //noInterrupts();
            Serial.flush();
            bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_SOFT_VER_LEN, RTX_TX);
            //interrupts();
            if (WriteStat) {
              uint32_t RTX_ReadTimeOut = micros();
          ...
          if (RTX_PROTOCAL.Connected()) {
            ...
            //noInterrupts();
            Serial.flush();
            bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_SOFT_DATE_LEN, RTX_TX);
            //interrupts();
            if (WriteStat) {
              uint32_t RTX_ReadTimeOut = micros();
              ...
              if (ResponseFound) {
                ...
                //noInterrupts();
                Serial.flush();
                bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_SOFT_TIME_LEN, RTX_TX);
                //interrupts();
                if (WriteStat) {
                  uint32_t RTX_ReadTimeOut = micros();
        ...
        if (ParseNext() == 0) {
          if (CharAvail == 2 && CharComp("CB", C, CharAvail)) {
            ...
            if (RTX_PROTOCAL.Connected()) {
              ...
              //              noInterrupts();
              Serial.flush();
              bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_CONFIG_BYTES_LEN, RTX_TX);
              //              interrupts();
              if (WriteStat) {
                uint32_t RTX_ReadTimeOut = micros();
                uint16_t TotalBytes;
            ...
            if (RTX_PROTOCAL.Connected()) {
              ...
              //              noInterrupts();
              Serial.flush();
              bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_DEBUG_BYTES_LEN, RTX_TX);
              //              interrupts();
              if (WriteStat) {
                uint32_t RTX_ReadTimeOut = micros();
                uint16_t TotalBytes;
            ...
            if (RTX_PROTOCAL.Connected()) {
              ...
              //              noInterrupts();
              Serial.flush();
              bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_EEPROM_LOAD_LEN, RTX_TX);
              //              interrupts();
              if (WriteStat) {
                uint32_t RTX_ReadTimeOut = micros();
            ...
            if (RTX_PROTOCAL.Connected()) {
              ...
              //              noInterrupts();
              Serial.flush();
              bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_EEPROM_BURN_LEN, RTX_TX);
              //              interrupts();
              if (WriteStat) {
                uint32_t RTX_ReadTimeOut = micros();
                bool ResponseFound = false;
                while (micros() - RTX_ReadTimeOut < RTX_RX_TIMEOUT && !ResponseFound) {
                  ...
                  if (RTX_PROTOCAL.Read(RX_IP, RX_Bytes, RX_MSG)) {
                    if (RX_IP == SlaveIP_Selected) {
                      //Corrrect IP
                      //Make sure it is System MSG and Ping Response:
                      if (RTX_PROTOCAL.RTX_MSG_TYPE[0] == 1 && RTX_PROTOCAL.RTX_MSG_TYPE[1] == 3 && RTX_PROTOCAL.RTX_MSG_TYPE[2] == 1 && RX_Bytes == 1) {
                        ResponseFound = true;
                      }
                    }
                  }
                ...
                if (ResponseFound) {
                  //Ping MSG Verified:
                  Serial.println(F("DEVICE EEPROM BURN RESPONSE || EEPROM BURNING"));
                  Serial.println();
                  Serial.println();
                  delay(500);
                }
            ...
            if (RTX_PROTOCAL.Connected()) {
              ...
              //              noInterrupts();
              Serial.flush();
              bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_EEPROM_STATUS_LEN, RTX_TX);
              //              interrupts();
              if (WriteStat) {
                uint32_t RTX_ReadTimeOut = micros();
                bool EEPROM_Error;
            ...
            if (RTX_PROTOCAL.Connected()) {
              ...
              //              noInterrupts();
              Serial.flush();
              bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_EEPROM_WRITE_COUNT_LEN, RTX_TX);
              //              interrupts();
              if (WriteStat) {
                uint32_t RTX_ReadTimeOut = micros();
          ...
          if (CharAvail == 3 && CharComp("CB ", C, CharAvail)) {
            ...
            if (Add > -1 && Add < 256) {
              if (ParseNext() == 0) {
                ...
                if (RTX_PROTOCAL.Connected()) {
                  ...
                  //              noInterrupts();
                  Serial.flush();
                  bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_CONFIG_READ_LEN, RTX_TX);
                  //              interrupts();
                  if (WriteStat) {
                    uint32_t RTX_ReadTimeOut = micros();
                    ...
                    if (ResponseFound) {
                      ...
                      if (BufferReadError) {
                        ...
                        Serial.print(F("READ: (ADD: ") P BufferRead[0] P F(", VAL: ") P BufferRead[1] P_ F(")"));
                      }
                      Serial.println();
                      Serial.println();
                      delay(500);
                ...
                if (ParseCharSingle() == ' ') {
                  int16_t Val = ParseVal();
                  if (Val > -1 && Val < 256) {
                    if (ParseNext() == 0) {
                      Serial.print(F("CONFIG-BUFFER WRITE (ADD: ") P Add P F(", VAL: ") P Val P_ F(")"));
                      Serial.println();
                      Serial.print(F("WRITING CONFIG-BUFFER (ADD: ") P Add P F(", VAL: ") P Val P F(") IP: ") P_ SlaveIP_Selected);
                      if (RTX_PROTOCAL.Connected()) {
                        uint8_t RTX_TX[RTX_REQ_CONFIG_WRITE_LEN] = RTX_REQ_CONFIG_WRITE(Add, Val);
                        //              noInterrupts();
                        Serial.flush();
                        bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_CONFIG_WRITE_LEN, RTX_TX);
                        //              interrupts();
                        if (WriteStat) {
                          uint32_t RTX_ReadTimeOut = micros();
                          ...
                          if (ResponseFound) {
                            Serial.print(F("DEVICE CONFIG-BUFFER WRITE RESPONSE || "));
                            if (BufferWriteError) {
                              Serial.print(F("ERROR: (ADD: ") P BufferWrite[0] P_ F(") OUT OF RANGE"));
                            }
                            else {
                              Serial.print(F("WRITE: (ADD: ") P BufferWrite[0] P F(", VAL: ") P BufferWrite[1] P_ F(")"));
                            }
                            Serial.println();
                            Serial.println();
                            delay(500);
            ...
            if (Add > -1 && Add < 256) {
              if (ParseNext() == 0) {
                ...
                if (RTX_PROTOCAL.Connected()) {
                  ...
                  //              noInterrupts();
                  Serial.flush();
                  bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_DEBUG_READ_LEN, RTX_TX);
                  //              interrupts();
                  if (WriteStat) {
                    uint32_t RTX_ReadTimeOut = micros();
                    uint8_t BufferRead[2];
                ...
                if (ParseCharSingle() == ' ') {
                  ...
                  if (Val > -1 && Val < 256) {
                    if (ParseNext() == 0) {
                      Serial.print(F("DEBUG-BUFFER WRITE (ADD: ") P Add P F(", VAL: ") P Val P_ F(")"));
                      Serial.println();
                      Serial.print(F("WRITING DEBUG-BUFFER (ADD: ") P Add P F(", VAL: ") P Val P F(") IP: ") P_ SlaveIP_Selected);
                      if (RTX_PROTOCAL.Connected()) {
                        uint8_t RTX_TX[RTX_REQ_DEBUG_WRITE_LEN] = RTX_REQ_DEBUG_WRITE(Add, Val);
                        //              noInterrupts();
                        Serial.flush();
                        bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_DEBUG_WRITE_LEN, RTX_TX);
                        //              interrupts();
                        if (WriteStat) {
                          uint32_t RTX_ReadTimeOut = micros();
                          uint8_t BufferWrite[2];
                          ...
                          if (ResponseFound) {
                            Serial.print(F("DEVICE DEBUG-BUFFER WRITE RESPONSE || "));
                            if (BufferWriteError) {
                              Serial.print(F("ERROR: (ADD: ") P BufferWrite[0] P_ F(") OUT OF RANGE"));
                            }
                            else {
            ...
            if (EEPROM_ErrorVal == 1 && ParseNext() == 0) {
              ...
              if (RTX_PROTOCAL.Connected()) {
                ...
                //              noInterrupts();
                Serial.flush();
                bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_EEPROM_SET_LEN, RTX_TX);
                //              interrupts();
                if (WriteStat) {
                  uint32_t RTX_ReadTimeOut = micros();
                  bool ResponseFound = false;
              ...
              if (RTX_PROTOCAL.Connected()) {
                ...
                //              noInterrupts();
                Serial.flush();
                bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_EEPROM_RESET_LEN, RTX_TX);
                //              interrupts();
                if (WriteStat) {
                  uint32_t RTX_ReadTimeOut = micros();
                  bool ResponseFound = false;
