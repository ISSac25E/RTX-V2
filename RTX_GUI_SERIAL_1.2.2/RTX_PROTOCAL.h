...
class RTX_PROTOCAL_class {
  ...
  public:
    ...
    uint8_t Write(uint8_t RecieverIP, uint8_t ByteCount, uint8_t *DataArray) {
      //Return Error Status:
      //[0] Write Success [1] Multi Write Try [2] Final Error
      uint8_t ReturnVal = 0;
      for (uint8_t X = 0; X < 2; X++) {
        if (!RTX.Write(RecieverIP, ByteCount, DataArray)) {
          if (!RTX.Status(6) || RTX.Status(0)) return ReturnVal;
        }
        else return(ReturnVal | B00000001);
        ReturnVal |= B00000010;
      }
      return (ReturnVal | B00000100);
    };
