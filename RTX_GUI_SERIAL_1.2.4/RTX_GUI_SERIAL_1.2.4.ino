...
#define RTX_SOFTWARE_VERSION {1,2,4}
...
uint8_t GUI_BUFF() {
  Serial.println(F("SELECT BUFFER COMMAND: CONFIG/DEBUG-BUFFER TB/R/W(C/DB (/# /#...)/DMP/CLR), EEPROM LOAD/BURN/STATUS/WRITE-CNT/RESET(E-L/B/S/WC/R#)"));
  while (1) {
    if (Serial.available()) {
      if (ParseNext() == 2) {
        char C[3] = {0, 0, 0};
        ...
        if (ParseNext() == 0) {
          ...
          if (CharAvail == 3 && CharComp("CB ", C, CharAvail)) {
            ...
            if (Add > -1 && Add < 256) {
              ...
              else if (CharAvail = 3 && CharComp("CLR", C, CharAvail) && ParseNext() == 0) {
                Serial.println(F("CONFIG-BUFFER DATA CLEAR"));
                Serial.print(F("CLEARING CONFIG-BUFFER DATA FROM IP ") P_ SlaveIP_Selected);
                if (RTX_PROTOCAL.Connected()) {
                  uint8_t RTX_TX[RTX_REQ_CONFIG_BYTES_LEN] = RTX_REQ_CONFIG_BYTES;
                  //              noInterrupts();
                  Serial.flush();
                  ...
                  if (WriteStat) {
                    ...
                    if (ResponseFound) {
                      ...
                      for (uint16_t X = 0; X < TotalBytes; X++) {
                        if (Serial.available()) {
                          if (ParseNext() == 2) {
                            ...
                            if (ParseNext() == 0) {
                              if (CharAvail == 1 && CharComp("C", C, CharAvail)) {
                                Serial.println();
                                Serial.println(F("DATA CLEAR CANCEL"));
                                Serial.println();
                                Serial.println();
                                return BUFF;
                              }
                        ...
                        if (RTX_PROTOCAL.Connected()) {
                          uint8_t RTX_TX[RTX_REQ_CONFIG_WRITE_LEN] = RTX_REQ_CONFIG_WRITE(X, 0);
                          //              noInterrupts();
                          Serial.flush();
                          bool WriteStat = RTX_PROTOCAL.Write(SlaveIP_Selected, RTX_REQ_CONFIG_WRITE_LEN, RTX_TX);
                          //              interrupts();
                      ...
                      Serial.println();
                      Serial.println();
                      Serial.print(F("CONFIG-BUFFER(BYTES: ") P TotalBytes P F(")") P_ F(" CLEARED"));
                      Serial.println();
                      Serial.println();
                      delay(500);
                    }
