/*
  RTX_PROTOCAL REV 1.0.10
  .h

  changes from 1.0.9:
    - changed command byte order:
        new:
          B01 = system command
          B11 = Buffer command
        old:
...
#include "Arduino.h"
#include "../RTX_ARDUINO_ONE_WIRE/RTX_ARDUINO_ONE_WIRE_V2_1.2/ONEWIRE_RTX.h"
#include "../RTX_EEPROM/RTX_EEPROM_1.0.2/RTX_EEPROM.h"
...
#define RTX_REQ_EEPROM_BURN_STAT B01001101
#define RTX_REQ_EEPROM_BURN_STAT_LEN 1
...
public:
  ...
  // Returns if any messages received:
  ...
  //[0]: 0 = System Command MSG, 1 = EEPROM Command MSG, 2 = General MSG Command, 4++ = ERROR:
      ...
      case B01:
        ...
        if (!(RX_MSG[0] & B00100000))
        {
          ...
          case 1:
            ...
            {
              uint8_t NameLength;
              if (sizeof(RTX_DEVICE_NAME) - 1 <= 5)
                NameLength = sizeof(RTX_DEVICE_NAME) - 1;
              else
                NameLength = 5;
              uint8_t TX_MSG[NameLength + 1];
              TX_MSG[0] = B00100100;
              for (uint8_t X = 0; X < NameLength; X++)
                TX_MSG[X + 1] = RTX_DEVICE_NAME[X];
          ...
          case 3:
            ...
            if (RX_BYTES == 1)
            {
              if (!(RX_MSG[0] & B01000000))
              {
                ...
                uint8_t TX_MSG[2];
                TX_MSG[0] = B10101101;
                TX_MSG[1] = RTX_EEPROM_SECTORS; // 32 Sectors to write
                RTX.Write(RX_IP, 2, TX_MSG);
              }
              else
              {
                // Request EEPROM Burn Status
                uint8_t TX_MSG[2];
                if (RTX_EEPROM_WRITING)
                {
                  TX_MSG[0] = B11101101;
                  TX_MSG[1] = (RTX_EEPROM.WriteProg() & B00111111); // How many Sectors are currently written so far
                }
                else
                {
                  TX_MSG[0] = B01101101;
          ...
          case 7:
            ...
            if (RX_BYTES == 1)
            {
              ...
              for (uint8_t X = 0; X < 4; X++)
                TX_MSG[4 - X] = (RTX_EEPROM.WriteCount() >> (8 * X));
