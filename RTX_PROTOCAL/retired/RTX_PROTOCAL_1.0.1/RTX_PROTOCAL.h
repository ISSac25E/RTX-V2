            ...
            switch ((RX_MSG[0] & B00011100) >> 2) {
              ...
              case 2:
                ...
                if (RX_BYTES == 1) {
                  ...
                  uint8_t VersionHold[3] = {0,0,0};
                  uint8_t VersionStringMarker = 0;
                  for(uint8_t X = 0; X < 3 && VersionStringMarker < sizeof(RTX_SOFTWARE_VERSION); X++) {
                    uint8_t StringValHold[3];
                    uint8_t StinrgValLength = 0;
                    for(uint8_t Y = 0; Y < 3; Y++) {
                      if(RTX_SOFTWARE_VERSION[VersionStringMarker] - '0' >= 0 && RTX_SOFTWARE_VERSION[VersionStringMarker] - '0' <= 9) {
                        StringValHold[StinrgValLength] = RTX_SOFTWARE_VERSION[VersionStringMarker] - '0';
                        StinrgValLength++;
                        VersionStringMarker++;
                      }
                      else {
                    ...
                    for(uint8_t Z = 0; Z < StinrgValLength; Z++){
                      VersionHold[X] += (StringValHold[StinrgValLength - (Z + 1)] * Multiplier);
                      Multiplier *= 10;
                    }
                  }
                  for(uint8_t X = 0; X < 3; X++) TX_MSG[X + 1] = VersionHold[X];
