#define RTX_DEVICE_NAME "SIDE"
#define RTX_SOFTWARE_VERSION {1,2,0}
#define RTX_DEVICE_IP 4 //3 for Center, 4 for side default
...
//5 Macros for each of the Modes:
PIN_MACRO ButtonMacro[5];
...
void BUTTON_RUN() {
  ...
  {
    //Run all 5 Macros
    static bool DelayMacroRun[5] = {false, false, false, false, false};
    for (uint8_t X = 0; X < 3; X++) {
      ButtonPin[X].Run();
    }
    for (uint8_t X = 0; X < 5; X++) {
      if (ButtonMacro[X].Run(RegButtonState(CONFIG_BUFF((8 * X) + (40 * (Mode % 4)))))) {
        //Button State Change
        if (ButtonMacro[X].State()) {
          //press/on/Lo
          RunMacro(CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 1 ));
        }
        else {
          //release/off/Hi
          RunMacro(CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 2 ));
          if (ButtonMacro[X].PrevInterval() >= (CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 7 ) | (CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 6 ) << 8))) {
            RunMacro(CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 5 ));
          }
          else {
            RunMacro(CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 4 ));
          }
          DelayMacroRun[X] = false;
        }
      }
      if (!DelayMacroRun[X] && ButtonMacro[X].State() && ButtonMacro[X].Interval() >= (CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 7 ) | (CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 6 ) << 8))) {
        RunMacro(CONFIG_BUFF( (8 * X) + (40 * (Mode % 4)) + 3 ));
  ...
  if (BTN_CMD & B00001111) {
    ...
    if (BTN_CMD & B10000000) {
      ...
      for (uint8_t X = 0; X < 4 && ButtonAND; X++) {
        if ((BTN_CMD & (1 << X))) {
          if (ButtonPin[X].ButtonState() != BTN_Trigger) ButtonAND = false;
        }
      }
      if (ButtonAND) return true;
    }
    else {
      //OR
      for (uint8_t X = 0; X < 4; X++) {
        if ((BTN_CMD & (1 << X))) {
          if (ButtonPin[X].ButtonState() == BTN_Trigger) return true;
        }
      }
    }
  }
...
void RunMacro(uint8_t Macro) {
  static uint32_t ButtonTimeOut = millis();
    ...
    case 1:
      {
        ...
        RTX_PROTOCAL.Write((CONFIG_BUFF(199) & B00000111), 3, TX_MSG);
        break;
      }
    case 2:
      //ATEM
...
void LED_RUN() {
  //Set which macro should be running here:
  {
    ...
    //Set Tally Macros:
    {
      ...
      switch (TallyStatus) {
        ...
        case 1:
          //PRG
          if (CONFIG_BUFF(175) & B00000100) TallyBuild.Macro = 1;
          else TallyBuild.Macro = 0;
          break;
        case 2:
          //PRE
          if (CONFIG_BUFF(175) & B00001000) TallyBuild.Macro = 2;
          else TallyBuild.Macro = 0;
          break;
        case 3:
          //PRG/PRE
  ...
  switch (StatusBuild.Macro) {
    ...
    case 1:
      ...
      if (StatusMacros[0].Run()) StatusMacros[0].Fade(CONFIG_BUFF(160 + (0 * 8) + 0), 12);
      if (StatusMacros[1].Run()) StatusMacros[1].Fade(CONFIG_BUFF(160 + (0 * 8) + 1), 12);
      if (StatusMacros[2].Run()) StatusMacros[2].Fade(CONFIG_BUFF(160 + (0 * 8) + 2), 12);
      break;
    case 2:
      if (StatusBuild.MacroChange()) {
        StatusMacros[0].RST();
      ...
      if (StatusMacros[0].Run()) StatusMacros[0].Fade(CONFIG_BUFF(160 + (1 * 8) + 0), 12);
      if (StatusMacros[1].Run()) StatusMacros[1].Fade(CONFIG_BUFF(160 + (1 * 8) + 1), 12);
      if (StatusMacros[2].Run()) StatusMacros[2].Fade(CONFIG_BUFF(160 + (1 * 8) + 2), 12);
      break;
    case 3:
      if (StatusBuild.MacroChange()) {
        StatusMacros[0].RST();
      ...
      if (StatusMacros[0].Run()) StatusMacros[0].Fade(CONFIG_BUFF(160 + (2 * 8) + 0), 12);
      if (StatusMacros[1].Run()) StatusMacros[1].Fade(CONFIG_BUFF(160 + (2 * 8) + 1), 12);
      if (StatusMacros[2].Run()) StatusMacros[2].Fade(CONFIG_BUFF(160 + (2 * 8) + 2), 12);
      break;
    case 4:
      if (StatusBuild.MacroChange()) {
        StatusMacros[0].RST();
      ...
      if (StatusMacros[0].Run()) StatusMacros[0].Fade(CONFIG_BUFF(160 + (3 * 8) + 0), 12);
      if (StatusMacros[1].Run()) StatusMacros[1].Fade(CONFIG_BUFF(160 + (3 * 8) + 1), 12);
      if (StatusMacros[2].Run()) StatusMacros[2].Fade(CONFIG_BUFF(160 + (3 * 8) + 2), 12);
      break;
  }
  ...
  switch (TallyBuild.Macro) {
    ...
    case 1:
      ...
      if (TallyMacros[0].Run()) TallyMacros[0].Fade(CONFIG_BUFF(163 + (0 * 8) + 0), 6);
      if (TallyMacros[1].Run()) TallyMacros[1].Fade(CONFIG_BUFF(163 + (0 * 8) + 1), 6);
      if (TallyMacros[2].Run()) TallyMacros[2].Fade(CONFIG_BUFF(163 + (0 * 8) + 2), 6);
      break;
    case 2:
      //PRE Tally
      if (TallyBuild.MacroChange()) {
      ...
      if (TallyMacros[0].Run()) TallyMacros[0].Fade(CONFIG_BUFF(163 + (1 * 8) + 0), 6);
      if (TallyMacros[1].Run()) TallyMacros[1].Fade(CONFIG_BUFF(163 + (1 * 8) + 1), 6);
      if (TallyMacros[2].Run()) TallyMacros[2].Fade(CONFIG_BUFF(163 + (1 * 8) + 2), 6);
      break;
    case 3:
      //PRG/PRE Mix Tally
      if (TallyBuild.MacroChange()) {
      ...
      if (TallyMacros[0].Run()) TallyMacros[0].Fade((CONFIG_BUFF(163 + (0 * 8) + 0) + CONFIG_BUFF(163 + (1 * 8) + 0)) / 2, 6);
      if (TallyMacros[1].Run()) TallyMacros[1].Fade((CONFIG_BUFF(163 + (0 * 8) + 1) + CONFIG_BUFF(163 + (1 * 8) + 1)) / 2, 6);
      if (TallyMacros[2].Run()) TallyMacros[2].Fade((CONFIG_BUFF(163 + (0 * 8) + 2) + CONFIG_BUFF(163 + (1 * 8) + 2)) / 2, 6);
      break;
  }
...
uint8_t ModeInc(uint8_t CurrMode) {
  ...
  for (uint8_t X = 0; X < 4 && !ModeFound; X++) {
    ...
    if (CONFIG_BUFF(193) & (1 << CurrMode)) ModeFound = true;
  }
  if (ModeFound) return CurrMode;
  else return 0;
}

bool ModeAvail() {
  if ((CONFIG_BUFF(193) & B10000000) && (CONFIG_BUFF(193) & B00001111)) return true;
  else return false;
}
